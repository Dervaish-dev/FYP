...
import { 
  ...
  ChevronUp, ChevronDown, BarChart3, LayoutGrid, List
} from 'lucide-react';
...
const CaregiverPatients = () => {
  ...
  const handleRegenerateInvite = async (inviteId) => {
    try {
      ...
      toast.success(`New invite code: ${data.invite?.code}`);
  ...
  const handleToggleInviteCode = async (inviteId) => {
    ...
    try {
      ...
      if (err.message && err.message.includes('legacy invite')) {
        await handleRegenerateInvite(inviteId);
      } else {
      ...
      <CaregiverLayout>
        <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: 'var(--theme-background)' }}>
          <div className="text-center">
            ...
            <p className="text-lg" style={{ color: 'var(--text-color)' }}>Loading patients...</p>
          </div>
        </div>
      </CaregiverLayout>
    );
  ...
  return (
    <CaregiverLayout>
      ...
      {/* Page Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold" style={{ color: 'var(--text-color)' }}>Patients</h1>
        <p className="mt-2 opacity-70" style={{ color: 'var(--text-color)' }}>Manage your patients and invites</p>
      </div>
      ...
      >
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center rounded-lg border p-1" style={{ borderColor: 'var(--border-color)' }}>
              <button
                onClick={() => setViewMode('grid')}
                className={`p-2 rounded-md transition-colors ${viewMode === 'grid' ? 'bg-opacity-10' : 'opacity-50 hover:opacity-100'}`}
                style={{ backgroundColor: viewMode === 'grid' ? 'rgba(var(--primary-rgb), 0.1)' : 'transparent' }}
                title="Grid View"
              >
                <LayoutGrid className="h-4 w-4" style={{ color: viewMode === 'grid' ? 'var(--primary-500)' : 'var(--text-color)' }} />
              </button>
              <button
                onClick={() => setViewMode('list')}
                className={`p-2 rounded-md transition-colors ${viewMode === 'list' ? 'bg-opacity-10' : 'opacity-50 hover:opacity-100'}`}
                style={{ backgroundColor: viewMode === 'list' ? 'rgba(var(--primary-rgb), 0.1)' : 'transparent' }}
                title="List View"
              >
                <List className="h-4 w-4" style={{ color: viewMode === 'list' ? 'var(--primary-500)' : 'var(--text-color)' }} />
              </button>
            </div>
            <button
            ...
            {patients.map((patient) => (
              ...
              >
                <div className="p-6">
                  ...
                  {/* Patient Stats Grid */}
                  <div className={`grid ${viewMode === 'grid' ? 'grid-cols-2 md:grid-cols-5' : 'grid-cols-2 md:grid-cols-5'} gap-4 mb-4`}>
                    <div>
